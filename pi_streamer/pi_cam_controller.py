...
class CameraStreamer:
    def __init__(self, stream: StreamSettings, server_capture: ServerCaptureSettings, config_path: Path) -> None:
        ...
        self._cleanup_failed_since: float | None = None
            ...
            if self._publish_process.poll() is None:
                ...
                try:
                    ...
                    try:
                        ...
                        raise RuntimeError("publisher process did not exit after SIGKILL")
            ...
            if self._capture_process.poll() is None:
                ...
                try:
                    ...
                    try:
                        ...
                        raise RuntimeError("capture process did not exit after SIGKILL")
        ...
        with self._lifecycle_lock:
            ...
            if stdout_thread is not None and stdout_thread is not threading.current_thread():
                ...
                if stdout_thread.is_alive():
                    ...
                    return self._incomplete_stop("previous stream pump did not stop")
                with self._lock:
            ...
            if not any(thread.is_alive() for thread in self._stderr_threads):
                ...
                return self._incomplete_stop("previous diagnostics reader did not stop")
            self._cleanup_failed_since = None
            with self._lock:
    ...
    def _incomplete_stop(self, reason: str) -> bool:
        self._set_stream_state("error", reason)
        now = time.monotonic()
        if self._cleanup_failed_since is None:
            self._cleanup_failed_since = now
        elif now - self._cleanup_failed_since >= 60:
            # Escalate to main's health check and systemd instead of leaving
            # the camera in an unrecoverable cleanup loop indefinitely.
            raise RuntimeError(f"stream cleanup timed out: {reason}")
        return False
...
class StreamSupervisor:
    ...
    def check_health(self) -> None:
        if not self._thread.is_alive() or time.monotonic() - self._heartbeat > 60:
            raise RuntimeError("stream supervisor stopped responding")
...
class MqttController:
    ...
    # Keep executable paths, destinations and pipeline options local-only.
    CAMERA_FIELDS = frozenset({
        "width", "height", "framerate", "bitrate", "sharpness",
    ...
    def check_health(self) -> None:
        if not self._worker.is_alive() or time.monotonic() - self._heartbeat > 60:
            raise RuntimeError("MQTT command worker stopped responding")
    ...
    def _handle_stream_set(self, payload: dict[str, Any]) -> None:
        ...
        forbidden = set(payload) - allowed - set(aliases)
        if forbidden:
            raise ValueError("unsupported MQTT settings (use local configuration): " + ", ".join(sorted(forbidden)))
        for source, target in aliases.items():
            if source in payload:
                if target in timelapse_changes:
                    raise ValueError(f"duplicate setting alias for {target}")
    ...
    def _handle_timelapse_set(self, payload: dict[str, Any]) -> None:
        ...
        if set(payload) - allowed:
            raise ValueError("unsupported timelapse settings")
        changes = {key: value for key, value in payload.items() if key in allowed}
        if not changes:
    ...
    def _on_message(self, client: mqtt.Client, userdata: Any, message: mqtt.MQTTMessage) -> None:
        ...
        try:
            if getattr(message, "retain", False):
                raise ValueError("retained commands are not accepted; publish commands with retain=false")
    ...
    def _run_commands(self) -> None:
        while not self._stop_event.is_set():
            ...
            try:
                ...
                # Bound sustained command execution and SD-card writes as well
                # as queue memory. Shutdown interrupts this wait immediately.
    ...
    def _execute_command(self, topic: str, payload_bytes: bytes) -> None:
        ...
        try:
            ...
            if topic == self._topic("cmd/start"):
                ...
                payload = self._decode_settings(payload_text)
    ...
    @staticmethod
    def _decode_settings(payload_text: str) -> dict[str, Any]:
        ...
        payload = json.loads(payload_text, object_pairs_hook=unique_fields, parse_constant=reject_constant)
        if not isinstance(payload, dict):
            raise ValueError("payload must be a JSON object")
        # Node's HTTP controller adds correlation metadata to every command.
        # Accept it explicitly, but never persist or treat it as a setting.
        if "_request_id" in payload:
            ...
            if (not isinstance(request_id, str) or not 1 <= len(request_id) <= 128
                ...
                raise ValueError("invalid _request_id")
        return payload
