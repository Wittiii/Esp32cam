...
void loadControllerSettings() {
  ...
  // This migration changes only the gain selection and its version marker;
  // preserve every other stored camera, exposure and IR setting.
  if (!dfrcamgain::persistMigration(
          configVersion,
          [&]() {
            return g_preferencesReady &&
                ((gainSettingExists && storedGainIndex == g_gainCeiling) ||
